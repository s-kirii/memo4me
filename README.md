...
- Electron の配布物を `dist-electron/v<version>/` に出力する
...
- macOS
  - `dist-electron/v1.0.3/mac-arm64/memo4me.app`
  - `dist-electron/v1.0.3/memo4me-1.0.3-mac-arm64.dmg`
- Windows
  - `dist-electron/v1.0.3/memo4me-1.0.3-win-arm64.exe`
  - `dist-electron/v1.0.3/memo4me-1.0.3-win-x64.exe`
...
デスクトップ版では、上記とは別に `dist-electron/v<version>/` 配下の `.app` / `.exe` / installer を配布対象とする。
...
- 設定メニューの `テーマ` から `Calm Editorial` / `Soft Editorial` / `Focus Light` / `Graphite Terminal` / `Girly` / `Starlight` / `Ocean` を切り替えられる
- テーマ設定は `localStorage` に保存され、次回起動時にも復元される
- 現在は `色 / 角丸 / 影 / 背景 / フォント / ボタン質感` を切り替える
