...
- 左ダッシュボードの状況サマリーは `今日やる` タスク専用とし、`今日のスプリント達成率` を単一指標で表示する
- 設定メニューから `全日ベース / 稼働日ベース` を切り替えられるようにする
- `稼働日ベース` は土日と日本の祝日を除いて `着手日〜期日` の総日数 / 経過日数を計算する
- 炎上予報は `週間 / 月間` を切り替え可能とし、日別の必要時間を表示する
- 右領域は `大きな詳細ペイン` ではなく、状態別の管理キャンバスとする
- 右領域では `未着手 / 進行中` を同時表示し、`完了` は補助モーダルで確認する
- 各タスク行は `今日やる表示チェック / タイトル / 期日 / 状態表示 / 進捗率表示 / タグ / 展開ボタン` を基本構成とする
- `今日やる` は手動設定ではなく、`着手日以上かつ未完了` なら自動で対象に含める
- タスクは `想定工数[H]` と `進捗率[%]` を持つ
- `進捗率` は `0%〜100%` を `5%` 刻みの数値ステッパーで設定する
- `0%=未着手 / 5-95%=進行中 / 100%=完了` として扱い、状態は進捗率から自動決定する
- 展開したタスクのみ、元メモ紐付け、進捗率、想定工数、着手目標、期日、タグ、2行メモを編集可能とする
- タスク詳細の `元メモ` から `メモ` ワークスペースへ戻れるようにする

状況サマリーロジック:

- `今日やる` かつ `完了前` かつ `着手日以上` のタスクを計算対象とする
- 各タスクについて、`T_ij` を `j日目時点の目標進捗率`、`P_ij` を現在の実進捗率、`E_i` を想定工数とする
- その日達成率は `Σ E_i(P_ij - T_i(j-1)) / Σ E_i(T_ij - T_i(j-1))` で計算する
- 分子がマイナスの場合は、昨日までの未達を今日へ持ち越している状態として扱う
- `着手日前` のタスクは計算対象外とし、件数だけ別表示する
- 想定工数未設定タスクは計算から外し、`未見積もり` 件数として補助表示する

炎上予報ロジック:

- 残工数は `想定工数 × (100 - 進捗率) / 100` で計算する
- `着手日以前` のタスクは `着手目標日〜期日` の日数で配分する
- `着手日以後` のタスクは `今日〜期日` の日数で配分する
- `期日当日` は残り日数を `1日` とする
- `期日超過` タスクも残り日数を `1日` とし、今日中に残工数を処理する前提で計算する
- 日別の必要時間合計が `5時間超` なら `注意`、`8時間超` なら `危険` とする
- `想定工数` 未設定タスクは予報計算から外し、`未見積もり件数` として表示する
