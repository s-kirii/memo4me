...
- タスクリストは実装済み
- AI アシスタントは実装済み
- タスク管理は `モーダル中心` から `独立ワークスペース` へ移行中
...
- 右領域:
  ...
  - タグ絞り込み
  - 並び替え
  - `未着手 / 進行中 / 完了` の 3 セクション同時表示
  - 各タスクは 1 行表示
  - 必要時のみ詳細展開

表示ルール:

- 左領域はデフォルトで簡易表示とし、詳細ダッシュボードは展開式にする
- 右領域は `大きな詳細ペイン` ではなく、状態別の管理キャンバスとする
- 右領域では `未着手 / 進行中 / 完了` を同時表示する
- 各タスク行は `完了チェックボックス / タイトル / 期日 / タグ / 展開ボタン` を基本構成とする
