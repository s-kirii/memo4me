...
- `Windows x64` の Electron デスクトップ版では設定メニューから更新確認できます
- `macOS` / `Windows ARM64` は手動更新です
...
1. 歯車ボタンから `進捗計算` を開きます
2. `全ての日で計算` または `稼働日で計算` を選びます
3. `稼働日で計算` を選ぶと、土日と日本の祝日を除いて目標進捗率を計算します

補足:

- この設定は、タスクワークスペース左側の `今日のスプリント達成率` に反映されます
- 設定はローカルに保存されます
...
1. 歯車ボタンから `更新` を開きます
2. `更新を確認` を押します
3. 新しい版がある場合は `ダウンロード` を押します
4. ダウンロード完了後、`再起動して更新` を押します

補足:

- この更新機能は現在 `Windows x64` の Electron デスクトップ版で利用できます
- `macOS` / `Windows ARM64` は GitHub Releases から新しい配布物を取得して手動更新します
...
- タスク一覧の左チェックは、`着手日以降なら今日やる対象` であることを示す自動表示です
- `今日やるのみ` フィルターで、着手日以降に入ったタスクだけを絞り込めます
- タスクには `想定工数[H]` と `進捗率[%]` を設定できます
- `進捗率` は `0%〜100%` を `5%` 刻みで操作し、`0%=未着手 / 5-95%=進行中 / 100%=完了` として扱われます
- 左ダッシュボードの `今日のスプリント達成率` は `今日やる` かつ `着手日以上` のタスクを対象に、単一指標で確認できます
- 各タスクの `想定工数 × (現在進捗率 - 前日目標進捗率)` を合計して、その日の目標幅で割った値を達成率として表示します
- 想定工数が入っていないタスクは `未見積もり` 件数として補助表示されます
...
`Windows x64` の Electron デスクトップ版では、設定メニューから更新確認できます。

それ以外の環境では、自動更新機能はまだありません。
