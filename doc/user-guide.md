...
3. デスクトップまたはスタートメニューの `memo4me` から起動します
...
- Electron 版では、この導線はウィンドウ終了とアプリ終了に直接つながります
...
- `進捗率` は `0% / 10% / ... / 100%` のトグルで設定し、`0%=未着手 / 10-90%=進行中 / 100%=完了` として扱われます
